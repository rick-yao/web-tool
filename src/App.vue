...
const tasks = ref<any[]>([]); // Store processing tasks
...
async function processAndUpload(file: File, task: any) {
  try {
    // 1. Process
    ...
    <header class="border-b">
      <div class="container mx-auto px-4 h-16 flex items-center justify-between">
        <div class="flex items-center gap-2">
           ...
           <h1 class="text-xl font-bold tracking-tight">Image Optimizer & Uploader</h1>
        </div>
        <SettingsDialog />
      </div>
    </header>
    ...
    <main class="container mx-auto px-4 py-8 max-w-3xl space-y-8">
      ...
      >
        <div class="flex flex-col items-center gap-4">
          <div class="h-16 w-16 bg-muted rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">
             <UploadCloud class="h-8 w-8 text-muted-foreground" />
          </div>
          <div>
            <h3 class="text-lg font-semibold">Drop images here</h3>
            <p class="text-sm text-muted-foreground mt-1">or click to select files</p>
          </div>
          <p class="text-xs text-muted-foreground">Supports JPG, PNG, WEBP (Auto converts to WEBP + AVIF)</p>
        </div>
      </div>
            ...
            <CardContent class="p-4">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-medium text-sm">{{ task.originalName }}</p>
                            <div class="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                                <span v-if="task.status === 'processing'" class="text-blue-500 flex items-center gap-1">
                                    <Loader2 class="h-3 w-3 animate-spin" /> Processing...
                                </span>
                                <span v-else-if="task.status === 'uploading'" class="text-yellow-500 flex items-center gap-1">
                                    <Loader2 class="h-3 w-3 animate-spin" /> Uploading to R2...
                                </span>
                                <span v-else-if="task.status === 'completed'" class="text-green-500 flex items-center gap-1">
                                    <Check class="h-3 w-3" /> Completed
                                </span>
                                <span v-else-if="task.status === 'error'" class="text-red-500">
                                    Failed: {{ task.error }}
                                </span>
                            </div>
                        </div>
                ...
                <Progress v-if="task.status !== 'completed' && task.status !== 'error'" :model-value="task.progress" class="h-1 mb-2" />

                <!-- Results -->
                <div v-if="task.status === 'completed'" class="grid gap-2 mt-4 bg-muted/30 p-3 rounded-md">
                    <div v-for="res in task.results" :key="res.url" class="flex items-center justify-between text-sm group/item">
                        <div class="flex items-center gap-2">
                             <span class="text-xs font-bold px-1.5 py-0.5 rounded bg-muted text-muted-foreground w-12 text-center">{{ res.type }}</span>
                             <a :href="res.url" target="_blank" class="text-muted-foreground truncate max-w-[200px] sm:max-w-[300px] hover:text-primary hover:underline">{{ res.url }}</a>
                        </div>
                        <Button 
                            variant="ghost" 
                            size="icon" 
